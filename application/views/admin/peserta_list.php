            ...
            <table class="table table-striped table-bordered table-hover table-condensed">
                ...
                <?php foreach($peserta as $row): ?>
                    ...
                    if ($row->status_pendaftaran == '0') {
                        ...
                        $pendaftaran = '<td class="status-biasa">' . anchor('admin/peserta/ubah-status-pendaftaran/'.$row->id, format_status_pendaftaran($row->status_pendaftaran), 'data-confirm="Mengubah status pendaftaran ke MUNDUR"') . '</td>';
                    }
                    ...
                    if ($row->status_verifikasi == '0') {
                        $verifikasi = '<td class="status-danger">' . anchor('admin/peserta/ubah-status-verifikasi/'.$row->id, format_status_verifikasi($row->status_verifikasi), 'data-confirm="Mengubah status verifikasi ke SUDAH?"') . '</td>';
                    } else {
                        $verifikasi = '<td class="status-biasa">' . anchor('admin/peserta/ubah-status-verifikasi/'.$row->id, format_status_verifikasi($row->status_verifikasi), 'data-confirm="Mengubah status verifikasi ke BELUM?"')  . '</td>';
                    }

                    // Status seleksi
                    if ($row->status_seleksi == '0') {
                        $seleksi = '<td class="status-danger">' . anchor('admin/peserta/ubah-status-seleksi/'.$row->id, format_status_seleksi($row->status_seleksi), 'data-confirm="Mengubah status seleksi ke LULUS?"') . '</td>';
                    } else {
                        $seleksi = '<td class="status-biasa">' . anchor('admin/peserta/ubah-status-seleksi/'.$row->id, format_status_seleksi($row->status_seleksi), 'data-confirm="Mengubah status seleksi ke TIDAK LULUS?"')  . '</td>';
                    }
